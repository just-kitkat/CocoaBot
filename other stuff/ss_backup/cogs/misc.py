...
"""
  ...
  async def cooldowns(self, ctx):
    if str(ctx.author.id) in self.bot.db["economy"]["users"]:
      ...
      balance_f = await self.bot.get_data("balance", ctx.author.id)
      ...
      if int(time.time()) - monthly > 86400*30:
        monthly = tick + " **READY**"
      else:
